...
(function () {
    ...
    function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions) {
        ...
        function makeSpritedButtonRow() {
            ...
            makeSpacer(1);
            buttons.link = makeButton("wmd-link-button", "链接 <a> Ctrl+L", "-40px", bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, false);
            }));
            buttons.quote = makeButton("wmd-quote-button", "引用 <blockquote> Ctrl+Q", "-60px", bindCommand("doBlockquote"));
            ...
            makeSpacer(2);
            buttons.olist = makeButton("wmd-olist-button", "数字列表 <ol> Ctrl+O", "-120px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, true);
            }));
            buttons.ulist = makeButton("wmd-ulist-button", "普通列表 <ul> Ctrl+U", "-140px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, false);
            }));
            buttons.heading = makeButton("wmd-heading-button", "标题 <h1>/<h2> Ctrl+H", "-160px", bindCommand("doHeading"));
            buttons.hr = makeButton("wmd-hr-button", "分割线 <hr> Ctrl+R", "-180px", bindCommand("doHorizontalRule"));
            makeSpacer(3);
